...
void false_Test(vector<QA> false_list) {
	...
	if (choice == 'Y' || choice == 'y') {
		for (int i = 0; i < (int)false_list.size(); i++) {
			...
			char ans;
			cin >> ans;
			if (ans == false_list[i].answer || ans == false_list[i].answer + 32) {
				std::cout << "回答正确！" << endl;
			}
			else if (ans == 'Q' || ans == 'q') {
				std::cout << "测试结束！" << endl;
			...
			else if (ans != 'A' && ans != 'B' && ans != 'C' && ans != 'D' && ans != 'Q' && ans != 'q') {
...
void start_Test(string filename) { //读取文件并开始做题
	...
	//操作说明
	std::cout << "╔═══════════════════════════操作说明═══════════════════════════════════════╗" << endl;
	std::cout << "║  1.输入答案时，请仅使用一个字母输入,不区分大小写，回车确认               ║ " << endl;
	...
	for (int i = 0; i < (int)qa_list.size(); i++) {
		...
		string ans;
		cin >> ans;
		if (ans[0] == qa_list[i].answer||ans[0] == qa_list[i].answer+32) {
			score++;
...
int main() {
	string filename = "C:\\Users\\C1\\Desktop\\期末\\大一上\\军事理论测试程序\\题目\\单选题 - GBK.txt";
	start_Test(filename);
