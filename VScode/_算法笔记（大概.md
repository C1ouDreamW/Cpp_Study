...
头文件`<functional>`中，包含less<int>和greater<int>，

大顶堆（默认值）：`priority_queue<int,vector<int>,less<int>>`

小顶堆： `priority_queue<int,vector<int>,greater<int> >`，或者直接把数按照其负数插入
...
### 6.快速乘
对于接近long long上限的数的快速幂，使用快速乘优化其中的乘法避免溢出
